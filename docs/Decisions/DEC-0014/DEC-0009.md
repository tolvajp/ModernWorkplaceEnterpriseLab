...
- MFA model targeting and assurance mapping (DEC-0013)
...
- if requirements are not met → access is blocked

Conditional Access does **not create entitlement**; it only constrains access.
...
- DEC-0013 defines acceptable MFA methods and assurance categories
...
- **MFA required**, at the assurance level defined by **DEC-0013**
  (i.e., allowed MFA methods and assurance categories are governed by DEC-0013)
...
Additional restrictions are permitted **only as explicit strictness above the baseline**, for example:
- restricting access modes
- requiring stronger security posture
- enforcing device-based conditions when applicable
...
High-confidence malicious activity signals may include a combination of the following
as determined by the security platform (e.g., SOC, identity protection, endpoint security):

- Identity-derived compromise verdict
- Endpoint-derived compromise verdict
- SOC-confirmed incident state

Implementation of complex compromise signals is **out of scope** for this lab.
The lab may initially rely on a limited subset of identity risk indicators.
...
security domains and is **not assumed** by this CA design. It may or may not be
