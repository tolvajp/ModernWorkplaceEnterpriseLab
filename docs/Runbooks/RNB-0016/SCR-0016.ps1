...
#Set tenant capability gating policies based on config file

foreach ($policyState in $config.PolicyStates) {
    try {
        Update-MgPolicyAuthenticationMethodPolicyAuthenticationMethodConfiguration @policyState
        Write-Host "Policy: $($policyState.AuthenticationMethodConfigurationId) state set to $($policyState.State)"
    } catch {
        Write-Host "Error setting policy state $($policyState.AuthenticationMethodConfigurationId) : $_"
}
}